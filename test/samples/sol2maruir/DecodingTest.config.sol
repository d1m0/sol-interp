...
contract DecodingTest {
    ...
    function testByte32() internal pure {
        bytes32 x = abi.decode(abi.encode(bytes32(0x0102030405060708091011121314151617181920212223242526272829303132)), (bytes32));
        assert(x == 0x0102030405060708091011121314151617181920212223242526272829303132);
    }

    function testInts() internal pure {
        (int256 a, uint256 b) = abi.decode(abi.encode(-123, 123), (int256, uint256));
        assert(a == (-123));
        assert(b == 123);
    }

    function testAddress() internal pure {
        address x = abi.decode(abi.encode(address(0xc03c4bF79eB0a0fD5fB75C35AddA741BC90Cf6b4)), (address));
    ...
    function testUintFixedArray() internal pure {
        uint256[3] memory x = abi.decode(abi.encode([uint256(1), 2, 3]), (uint256[3]));
        assert(x[0] == 1);
        assert(x[1] == 2);
        assert(x[2] == 3);
    }
