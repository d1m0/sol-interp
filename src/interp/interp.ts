...
import {
    ...
    bytesT,
    bytesToIntOfType,
    castBytesViewToString,
    castStringViewToBytes,
    cdBytesT,
    changeLocTo,
    clampIntToType,
    decodeView,
...
export class Interpreter {
    ...
    constructor(
        ...
        this.nodes = [];
        this.ctx = artifact.ctx;
        this.arrayBuiltins = makeBuiltin(
            arrayBuiltinDecs,
    ...
    astToRuntimeType(t: sol.TypeIdentifier, loc?: sol.DataLocation): BaseInterpType {
        return typeIdToRuntimeType(t, this.ctx, loc);
    }

    varDeclToRuntimeType(decl: sol.VariableDeclaration): BaseInterpType {
        return typeIdToRuntimeType(sol.typeOf(decl), this.ctx);
    }

    typeOf(e: sol.Expression): BaseInterpType {
        return this.astToRuntimeType(sol.typeOf(e));
    }
    ...
    private getCalldataArgsAndTypes(
        ...
        if (entryPoint instanceof sol.FunctionDefinition) {
            ...
            argTs = entryPoint.vParameters.vParameters.map((argT) => {
                ...
                if (isLib && isStoragePointerOrMap(declT)) {
                    return declT;
                }

                return changeLocTo(declT, sol.DataLocation.CallData);
            });
        } else {
            argTs = getGetterArgAndReturnTs(entryPoint)[0];
        }

        const calldataArgs: PrimitiveValue[] = calldataViews.map((view, i) => {
            const argT = argTs[i];

            if (isStoragePointerOrMap(argT)) {
                const v = view.decode(data);
                sol.assert(typeof v === "bigint", `Expected bigint for struct pointer not ${v}`);
                const viewT = argT instanceof rtt.PointerType ? argT.toType : argT;
                return rtt.makeStorageView(viewT, [v, 32]);
            }
    ...
    public call(msg: SolMessage, state: State): Uint8Array | RuntimeError {
        ...
        try {
            if (entryPoint instanceof sol.FunctionDefinition) {
                ...
                resTs = getGetterArgAndReturnTs(entryPoint)[1];
            }
        } catch (e) {
            if (e instanceof RuntimeError) {
                return e;
        ...
        if (
            ...
            // Handle storage pointers/maps in returns
            [res, resTs] = encodeStoragePtrsForABI(res, resTs);
    ...
    public callGetter(
        ...
        const returnVals: Value[] = returnViews.map((v) => {
            if (isValueType(v.type)) {
                ...
                this.expect(
                    isPrimitiveValue(t),
                    `Expected a primitive value not: ${t}, ${t.constructor.name}`
                );
                return t;
            }
    ...
    private execRevertStatement(stmt: sol.RevertStatement, state: State): ControlFlow {
        ...
        const argExprs = this.getFunCallArgs(stmt.errorCall, errorDef);
        const [data, msg] = this.evalCustomError(errorDef, argExprs, state);
    ...
    evalLV(expr: sol.Expression, state: State): LValue {
        ...
        if (expr instanceof sol.Identifier) {
            ...
            // A bit of a hack to handle refering to state variables with ParentContract.Var
            if (
                expr.vReferencedDeclaration instanceof sol.VariableDeclaration &&
                expr.vReferencedDeclaration.stateVariable
            ) {
                this.expect(state.scope !== undefined, `Missing scope in evalLV()`);
                const varView = state.scope.lookupLocation(expr.vReferencedDeclaration);
                this.expect(varView !== undefined);
                res = varView;
            } else {
            ...
            if (
                ...
                this.expect(
                    arr instanceof rtt.ArrayStorageView || arr instanceof rtt.BytesStorageView
                );

                res = this.pushImpl(arr, undefined, state);
            } else {
                const callResult = this.evalNP(expr, state);
                this.expect(
                    callResult instanceof View,
                    `Unexpected non-view value in evalLV as result of call to ${expr.vFunctionName}: ${callResult}`
                );
                res = callResult;
            }
        } else {
    ...
    private implicitCoercion(
        ...
        if (
            typeof rvalue === "bigint" &&
            lvType instanceof RationalNumberType &&
            rvalue === lvType.numerator &&
            lvType.denominator === 1n
        ) {
            return rvalue;
        }
    ...
    private coerceToSameType(
        ...
        if (
            lType instanceof RationalNumberType &&
            rType instanceof RationalNumberType &&
            typeof left === "bigint" &&
            typeof right === "bigint"
        ) {
            return [left, right];
    ...
    evalBinaryOperation(expr: sol.BinaryOperation, state: State): Value {
        ...
        if (typ instanceof RationalNumberType) {
            this.expect(typ.isInt(), `Unexpected non-integer constant`);
            return typ.asInt();
        }
    ...
    evalTypeConversion(expr: sol.FunctionCall, toT: rtt.BaseRuntimeType, state: State): Value {
        ...
        // This is also a 'no-cast' case
        // @todo is this still neccessary? Feels hacky. Try removing it
        if (
            fromT instanceof rtt.PointerType &&
            fromT.location === sol.DataLocation.Memory &&
            fromT.toType.pp() == toT.pp()
        ) {
            this.expect(fromV instanceof BaseMemoryView);
            return fromV;
        }
        ...
        if (
            ...
            return castBytesViewToString(fromV);
        }
        ...
        if (
            ...
            this.expect(bts instanceof Uint8Array, `Expected value to be Uint8Array`);
        ...
        // library -> address
        if (
            fromT instanceof WrappedType &&
            fromT.innerT instanceof sol.TypeTypeId &&
            fromT.innerT.actualT instanceof sol.ContractTypeId &&
            toT instanceof rtt.AddressType
        ) {
            const contractDef = this.ctx.locate(fromT.innerT.actualT.id);
            if (
    ...
    /**
     * Given a function call get the argument expressions in the correct order.
     * In the presence of fieldNames - match up the filed names with the formal call parameters/struct field order.
     *
     * Note: target may be undefined for internal calls to function variables.
     */
    getFunCallArgs(
        expr: sol.FunctionCall,
        target:
        ...
        if (target === undefined || expr.fieldNames === undefined || expr.vArguments.length === 0) {
            return expr.vArguments;
        }

        let formals: string[];

        if (target instanceof sol.FunctionDefinition) {
            formals = target.vParameters.vParameters.map((v) => v.name);
        } else if (target instanceof sol.EventDefinition) {
            formals = target.vParameters.vParameters.map((v) => v.name);
        } else if (target instanceof sol.ErrorDefinition) {
            formals = target.vParameters.vParameters.map((v) => v.name);
        } else if (target instanceof sol.ContractDefinition) {
            this.expect(
                target.vConstructor !== undefined,
                `Unexpected constructor reference with fieldNames to ${target.name} with no constructor`
            );
            formals = target.vConstructor.vParameters.vParameters.map((v) => v.name);
        } else {
            formals = target.fields
                .filter(([, typ]) => !skipFieldDueToMap(typ))
                .map(([name]) => name);
        }

        const filedNameToInd = new Map<string, number>(expr.fieldNames.map((n, i) => [n, i]));

        const res: sol.Expression[] = [];
        for (const name of formals) {
            const idx = filedNameToInd.get(name);
            // Its possible for a formal to be not amongst `expr.vArguments` when its in the receiver - eg:
            // In `address.safeTransferFrom(...)` in the presence of `using SafeToken for address`, the `from` field of `safeTransferFrom` is not
            // in vArguments
            if (idx !== undefined) {
                res.push(expr.vArguments[idx]);
            }
        }

        this.expect(res.length === expr.vArguments.length, `Missed an argument in fieldNames call`);
    ...
    evalStructConstructorCall(
        ...
        const assignedFields = structT.fields.filter(([, typ]) => !skipFieldDueToMap(typ));

        // Note that we evaluate fields in their struct declaration order
        const exprs = this.getFunCallArgs(expr, structT);

        for (let i = 0; i < exprs.length; i++) {
            const fieldVal = this.evalNP(exprs[i], state);
            const [fieldName] = assignedFields[i];
    ...
    private _evalMsgCall(
        ...
        if (extCallDesc.target instanceof rtt.ExternalFunRef) {
            ...
            if (astTarget instanceof sol.FunctionDefinition) {
                ...
                [argTs] = getGetterArgAndReturnTs(astTarget);
            } else {
                nyi(`External call target ${astTarget?.print()}`);
            }

            // Next compute the msg data
            const argExprs: sol.Expression[] =
                astTarget instanceof sol.FunctionDefinition
                    ? this.getFunCallArgs(expr, astTarget)
                    : expr.vArguments;

            let args = argExprs.map((arg) => this.eval(arg, state));

            if (callee instanceof CurriedVal) {
                args.unshift(...callee.args);
            }

            // Handle storage/map pointers
            [args, argTs] = encodeStoragePtrsForABI(args, argTs);
            ...
            this.expect(contract instanceof sol.ContractDefinition);
            const argExprs = this.getFunCallArgs(expr, contract);

            const args = argExprs.map((arg) => this.eval(arg, state));
    ...
    private getValuesFromReturnedCalldata(
        ...
        // The actual ABI return type. For storage pointers/maps this differs from the
        // corresponding return type.
        const rawRetTs: rtt.BaseRuntimeType[] = [];
        const retTs: rtt.BaseRuntimeType[] = [];

        if (target instanceof sol.FunctionDefinition) {
            for (const retDecl of target.vReturnParameters.vParameters) {
                const astType = this.varDeclToRuntimeType(retDecl);
                const retType = isStoragePointerOrMap(astType)
                    ? astType
                    : changeLocTo(astType, sol.DataLocation.Memory);
                const rawType = isStoragePointerOrMap(astType) ? rtt.uint256 : retType;

                retTs.push(retType);
                rawRetTs.push(rawType);
            }
        } else if (target instanceof sol.VariableDeclaration) {
            this.expect(target.stateVariable);
            const [, getterRetT] = getGetterArgAndReturnTs(target);
            retTs.push(...getterRetT);
            rawRetTs.push(...getterRetT);
        }

        const rawVals = this.assertNotPoison(state, decode(data, rawRetTs, state));

        return rawVals.map((v, i) => {
            const retT = retTs[i];
            if (!isStoragePointerOrMap(retT)) {
                return v;
            }

            sol.assert(typeof v === "bigint", ``);

            return rtt.makeStorageView(retT, [v, 32]);
        });
    }
    ...
    _execCall(
        ...
        let callTarget: sol.FunctionDefinition | sol.ModifierDefinition | BuiltinFunction;
        let actualCallee: sol.FunctionDefinition | sol.ModifierInvocation | BuiltinFunction;
        ...
        let scope: BaseScope;
        try {
            this.nodes.push(callTarget);
            scope = this.makeScope(callTarget, args, argTs, state);
        } finally {
            this.nodes.pop();
        }

        return this._execCallInt(actualCallee, scope, state);
    }
    ...
    evalInternalCall(expr: sol.FunctionCall, state: State): Value {
        ...
        if (codeAccount.artifact.codegen === "old") {
            argVals = argExprs.map((arg) => this.eval(arg, state));
    ...
    evalEventCall(expr: sol.FunctionCall, state: State): Value {
        ...
        const argExprs = this.getFunCallArgs(expr, evt);
        const args = argExprs.map((arg) => this.evalNP(arg, state));
    ...
    evalErrorCall(expr: sol.FunctionCall, state: State): Value {
        ...
        const argExprs = this.getFunCallArgs(expr, callee.def);
        const [errorBytes] = this.evalCustomError(callee.def, argExprs, state);
    ...
    evalIndexRangeAccess(expr: sol.IndexRangeAccess, state: State): Value {
        ...
        if (start === end) {
            //@todo the offset here is wrong. It shouldn't matter much, as we are reading 0 bytes, but it may matter
            //if its introspected by assembly. Tracked under #114
            return new rtt.BytesSliceCalldataView(base.type, base.base + base.offset, 0n);
        }

        const rangeOffset = base.indexView(start, cd);
        this.expect(
            rangeOffset instanceof BaseCalldataView,
            `evalIndexRangeAccess: expected base to be BaseCalldataView not ${ppValue(rangeOffset)}`
        );
    ...
    evalLiteral(expr: sol.Literal, state: State): Value {
        if (expr.kind === sol.LiteralKind.Number) {
            ...
            return type instanceof rtt.AddressType
    ...
    evalBuiltinMemberAccess(
        ...
        // We do this check first since we may wrap the bultin function in a CurriedVal
        const shouldCallImmediately = val instanceof BuiltinFunction && val.isField;
        ...
        if (shouldCallImmediately) {
            const res = this._execCall(val as CurriedVal | BuiltinFunction, [], [], state);
    ...
    evalMemberAccess(expr: sol.MemberAccess, state: State): Value {
        ...
        if (
            ...
            if (
                ...
                this.expect(def.vScope instanceof sol.ContractDefinition);
                const libAddr = getLibraryLinkedAddress(def.vScope, state, this.artifactManager);
                this.expect(
                    libAddr !== undefined,
                    `Missing address for linked library ${def.vScope.name}`
                );
        ...
        if (baseVal instanceof Address) {
            ...
            if (
                ...
                const selector = sol.signatureHash(def);
    ...
    /**
     * Push a new value (or the 0-value if undefined passed) on a storage array/bytes and return a View to the new value.
     * @param array
     * @param value
     * @param state
     */
    pushImpl(
        arr: rtt.ArrayStorageView | rtt.BytesStorageView,
        value: Value | undefined,
        state: State
    ): View {
        const elT = arr instanceof rtt.ArrayStorageView ? arr.type.elementT : bytes1;
        const curSize = arr.size(getStateStorage(state));

        this.expect(!isFailure(curSize));
        value = value === undefined ? makeZeroValue(elT, state) : value;

        if (arr instanceof rtt.ArrayStorageView) {
            const sizeView = new rtt.IntStorageView(rtt.uint256, [arr.key, arr.endOffsetInWord]);
            ...
            if (elView instanceof DecodingFailure) {
                this.fail(InternalError, `push(): couldn't get new element view`);
            }
            ...
            if (bytes instanceof DecodingFailure) {
                this.fail(InternalError, `push(): couldn't decode bytes`);
            }

            const newByte = value instanceof Uint8Array ? value : new Uint8Array([Number(value)]);

            bytes = concatBytes(bytes, newByte);
            setStateStorage(state, arr.encode(bytes, storage));
        }

        const res = arr.indexView(curSize, getStateStorage(state));
        this.expect(!isFailure(res));
        return res;
    }
}
