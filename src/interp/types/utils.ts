...
export function astToInterpType(raw: sol.TypeNode, infer: sol.InferType, loc?: sol.DataLocation): interp.BaseType {
    ...
    if (simplified instanceof sol.ArrayType) {
        return new interp.ArrayType(astToInterpType(simplified.elementT, infer, loc), simplified.size);
    }

    if (simplified instanceof sol.FunctionType) {
        sol.assert(simplified.visibility === sol.FunctionVisibility.External || simplified.visibility === sol.FunctionVisibility.Internal, `Invalid fun type visiblity ${simplified.visibility}`);
        ...
        return new (simplified.visibility === sol.FunctionVisibility.External ? interp.ExternalFunctionType : interp.InternalFunctionType)(paramTs, retTs);
    }
    ...
    if (simplified instanceof sol.PointerType) {
        return new interp.PointerType(astToInterpType(simplified.to, infer, simplified.location), simplified.location);
    }
