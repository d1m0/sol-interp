import { Address, concatBytes, createAddressFromBigInt, createAddressFromString, hexToBytes } from "@ethereumjs/util";
import { Chain } from "../interp";
import { ArtifactManager } from "../interp/artifactManager";
import { TraceVisitor } from "../interp/visitors";
import { ParsedStep } from "./ast/parser";
import { address, AddressType, astToRuntimeType, ContractInfo, IntType, nyi, Value, ZERO_ADDRESS } from "sol-dbg";
...
export class Runner {
    ...
    constructor(private readonly artifactManager: ArtifactManager) {
        ...
        this.chain.makeEOA(this.SENDER, 1000000000000n);
    }
    ...
    evalExpr(expr: ExpressionNode, type: BaseInterpType): Value {
        if (type instanceof IntType) {
            this.expect(expr.kind === "HexNumber" || expr.kind === "DecNumber", `Expected a number for ${type.pp()} not ${expr.kind}`);
            return expr.value
        }

        if (type instanceof AddressType) {
            this.expect(expr.kind === "Var" || expr.kind === "HexNumber", `Expected a number for ${type.pp()} not ${expr.kind}`);
    ...
    findMethod(contract: sol.ContractDefinition, name: string): sol.FunctionDefinition | sol.VariableDeclaration {
        for (const base of contract.vLinearizedBaseContracts) {
            for (const method of base.vFunctions) {
                if (![sol.FunctionVisibility.External, sol.FunctionVisibility.Public].includes(method.visibility)) {
                    continue
                }
    ...
    getMsgFromStep(step: ParsedStep): [SolMessage, ContractInfo] {
        ...
        let argRTTs = argSolTs.map((solT) => astToRuntimeType(solT, infer, sol.DataLocation.Memory))
        this.expect(argRTTs.length === step.args.length, `Mismatch in given arg length (${step.args.length} and number of args (${argRTTs.length} for method ${target ? target.name : "<unknown>"} in ${targetContract.contractName}))`)

        let args = step.args.map((exprNode, i) => this.evalExpr(exprNode, argRTTs[i]));
        let abiTypeNames = argSolTs.map(sol.abiTypeToCanonicalName)

        const argData = args.length > 0 ? hexToBytes(ethABI.encodeParameters(abiTypeNames, args) as `0x${string}`) : new Uint8Array()
        let data: Uint8Array
        ...
        if (step.kind === "Deploy") {
            ...
            let selector = hexToBytes(`0x${infer.signatureHash(target)}`)
    ...
    run(step: ParsedStep): CallResult {
